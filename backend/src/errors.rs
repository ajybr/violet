...
impl From<DbError> for HTTPError {
    fn from(err: DbError) -> Self {
        let (status, detail) = match err {
            DbError::UniqueViolation => (StatusCode::CONFLICT, "Unique constraint violated"),
            DbError::NotFound => (StatusCode::NOT_FOUND, "Not found"),
            DbError::DimensionMismatch => (StatusCode::BAD_REQUEST, "Dimension mismatch"),
            _ => (StatusCode::INTERNAL_SERVER_ERROR, "Internal server error"),
        };
        HTTPError::new(detail).with_status(status)
    }
}
